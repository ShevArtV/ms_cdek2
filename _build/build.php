...
class ms_CDEK2Package
{
    ...
    protected function settings()
    {
        ...
        // Add properties for msDelivery
        $areas = ['ms2_delivery_mscdekhandler' => '_tarif', 'ms2_delivery_mscdekhandlerpvz' => '_tarifpvz'];
        foreach ($areas as $area => $key) {
            $setting = $this->modx->newObject('modSystemSetting');
            $setting->fromArray([
                'key' => $this->config['name_lower'] . $key,
                'name' => 'setting_' . $this->config['name_lower'] . $key,
                'namespace' => 'minishop2',
                'area' => $area,
                'xtype' => 'textfield',
                'value' => '',
...
$install = new ms_CDEK2Package(MODX_CORE_PATH, $config);
